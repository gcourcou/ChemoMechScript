...
sheet = Sheet.planar_sheet_2d('basic2D', nx=int(nx), ny=int(ny), distx=1, disty=1,noise=pos_noise)
...
sheet=sheet.extract_bounding_box_GC_2dellipse(
    rx, ry, coords=["x", "y"] )
...
def Dif(t,y,i,N_p,mag):
    ...
    for index, row in local_edge_df.iterrows():
        ...
        # otherwise add diffusion propotional to length
            ...
            term+= -(y[i+N_p*len(sheet.face_df)]-y[ sheet.edge_df.at[row["opposite"],'face'] +N_p*len(sheet.face_df) ])*mag[N_p]
    #print (term)
    return term
...
def cell(t, y):
    ...
    for N_p, p_name in proteins.items():
        for index, row in sheet.face_df.iterrows():
            fun+= [Dif(t,y,index,int(N_p),Dif_mag)+Decay(t,y,index,int(N_p),Decay_mag) + Auto_Production(t,y,index,int(N_p),row,auto_prod_mag) ]
        #print ("fun")
        #print (fun)
    return fun
...
def animate_cells_MF(timer,chem_name,string):
    # String tells us how to name file after image
    plt.clf()
    sheet.face_df['col'] = np.array([ int(row['population_variable']=='MF')*1.  for index, row in sheet.face_df.iterrows() ])
    #sheet.face_df['col'] = sheet.face_df[chem_name]
    cmap = plt.cm.get_cmap('viridis')
    color_cmap = cmap(sheet.face_df.col)
    draw_specs['face']['color'] = color_cmap
...
def plot_chem(timer,chem_name,string):
    # String tells us how to name file after image
    plt.clf()
    x_y_array = np.array([ [row['x'],row['y'],row[chem_name]]  for index, row in sheet.face_df.iterrows() ])
    plt.plot([i[0] for i in x_y_array],[i[2] for i in x_y_array],'bx',alpha=0.25)
    x_y_ref=[]
    val=(sheet.face_df['y'].max()+sheet.face_df['y'].min() )/2.
    for item in x_y_array:
        if val-1.<item[1]<val+1.:
            x_y_ref.append(item)
...
def plot_topology(timer,sheet):
    ...
    for index,row in sheet.face_df.iterrows():
        sheet.face_df.at[index,'rlarea'] = sheet.face_df.at[index,'area']/sheet.face_df['area'].mean()
...
def mf_inf_cell(sheet, lower_limit, upper_limit, hh_inf_area,relax_area,string='y_concentration'):
...
def cell_grow_and_divide(tyssue):
    ...
    cell_GS(tyssue,amin,amax,gamma_G,gamma_S,dt)
...
store_MF_position=[]
store_mech_timer=[]
def data_collection(i,tyssue,cell_number,MF_position,mech_timer):
    cell_number+=[ tyssue.face_df.shape[0]  ]
    x_y_array = np.array([ [row['x'],row['y'], row['population_variable']=='MF']  for index, row in tyssue.face_df.iterrows() ])
    x_y_ref=[]
    val=(tyssue.face_df['y'].max()+tyssue.face_df['y'].min() )/2.
    for item in x_y_array:
        if val-1.<item[1]<val+1. and item[2]==True :
...
def chemo_mech_iterator(sheet,initial_concentration, derivative_function, mechanical_reaction,cell_grow_and_divide,visualization,**kwargs):
    ...
    for i in range(0,steps):
        ...
        # reaction diffusion
        sol = solve_ivp(derivative_function, [kwargs['t_mech']*i, kwargs['t_mech']*(i+1)], initial_concentration)
        sol_last_concentration=[item[-1] for item in sol.y]
        # put chem result in tissue object
        for N_p, p_name in proteins.items():
            sheet.face_df[p_name]=sol_last_concentration[len(sheet.face_df)*int(N_p):len(sheet.face_df)*(int(N_p)+1)]
