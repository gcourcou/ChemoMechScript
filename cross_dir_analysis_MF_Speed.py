...
from import_analyze_script_out_MF_Speed import analyze
...
for directory in targets:
    if directory[-1]!="_":
        ...
        # touple
        param=np.around(temp['parameters'][parameter_in_question],decimals=1)
        store[param]=temp
        #print(os.listdir())
...
x_data = np.arange(0.,3.,.3)
x_data=np.around(x_data,decimals=1)
...
mag_colour_data={}
color_saturation_c=min(x_data)
color_min_c=max(x_data)
for item_x in x_data:
...
for key in plot_keys_vectors:
    ...
    for item_x in x_data:
        ...
        time_array=np.arange(0,len(store[item_x][key]))*915.3565322296259*0.000277778
