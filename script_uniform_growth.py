...
script_data_keys=["total real time","cell number","tissue area","MF position","Mech Timer","mitotic position","L",
                  ...
                  ,"radial_shape_plot","shape_distribution","shape_average_area",
...
parameters = {}
with open("parameters_uniform.txt") as f:
    for line in f:
        (key, val) = line.split()
        # print(line.split())
        try:
...
#    
    ...
    for i in range(0,radial_resolution-1):
        ...
        radial_shape[i]=np.average(radial_shape[i])
    script_data["radial_area_plot"]+=[[radial_area_pos,radial_area]]
    script_data["radial_shape_plot"]+=[[radial_area_pos,radial_shape]]
